...
void gridToLatLon(const char* mgrid, double &lat, double &lon) {
  ...
  lon = (grid[0] - 'A') * 20.0 - 180.0 + 1.0;
  lat = (grid[1] - 'A') * 10.0 - 90.0 + 0.5;
  if (grid.length() >= 4) {
    lon += (grid[2] - '0') * 2.0;
    lat += (grid[3] - '0') * 1.0;
  }
  if (grid.length() >= 6) {
    lon += (tolower(grid[4]) - 'a') * (2.0/24.0);
    lat += (tolower(grid[5]) - 'a') * (1.0/24.0);
  }
}
...
void openSetupPortal() {
  ...
  WiFiManagerParameter param_norad("norad", "NORAD ID", selectedNorad.c_str(), 8);
  ...
  M5.Display.drawString("Connect to:", 80, 160);
  ...
  if (wm.startConfigPortal("M5PaperS3-Setup")) {
    ...
    if (gridStr.length() >= 4) {
      gridToLatLon(gridStr.c_str(), qth_lat, qth_lon);
    } else {
      qth_lat = atof(param_lat.getValue());
      qth_lon = atof(param_lon.getValue());
    }
    selectedNorad = param_norad.getValue();
    if (selectedNorad.length() > 0) selectedName = "NORAD " + selectedNorad;
    saveConfig();
  }
  WiFi.begin();
}
...
void predictPasses() {
  ...
  while (now < future && passCount < 8) {
    if (sat.predict(overpass, now, 5.0)) {   // min elevation 5°
...
void updateCurrentPosition() {
  time_t now = time(nullptr);
  double jd = sat.julian(now);   // Convert unix time to JD
  sat.findsat(jd);
}
...
// ... (rest of the code: drawSatSelectScreen, wasTouched, handleTouch, setup, loop) remains the same as previous version
...
void loop() {
  M5.update();
  // handleTouch() function from previous version
